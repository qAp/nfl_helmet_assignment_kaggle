...
def check_submission(sub):
    ...
    has_duplicate_boxes = sub[["video_frame", "left",
                               "width", "top", "height"]].duplicated().any()
...
def force_sub_requirements(sub, verbose=True):
    ...
    *Warning* Using this code may remove prediction rows in a sub-optimal manner.
    ...
    sub = sub.drop_duplicates(
        ['video_frame', 'left', 'width', 'top', 'height'])
    ...
    if verbose:
        ...
        print(
            f'Forcing submission requirements removed {n_removed} rows ({len_before} -> {len_after})')
    return sub
...
class NFLAssignmentScorer:
    def __init__(
        ...
        if remove_sideline:
            self.labels_df = self.labels_df.query(
                "isSidelinePlayer == False").reset_index(drop=True).copy()
    ...
    def merge_sub_labels(self, sub, labels, weight_col="isDefinitiveImpact"):
        ...
        base_columns = ["label", "video_frame", "x1", "x2",
                        "y1", "y2", "left", "width", "top", "height"]
    ...
    def score(self, sub, labels_df=None, drop_extra_cols=True):
        ...
        if drop_extra_cols:
            drop_cols = ["x1_sub", "x2_sub", "y1_sub",
                         "y2_sub", "x1_gt", "x2_gt", "y1_gt", "y2_gt"]
