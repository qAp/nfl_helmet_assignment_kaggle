...
def video_with_predictions(
    ...
    sub_labels['video'] = sub_labels['video_frame'].str.split(
        '_').str[:3].str.join('_')
    sub_labels['frame'] = sub_labels['video_frame'].str.split(
        '_').str[-1].astype('int')
    ...
    while True:
        ...
        cv2.putText(img, img_name, (5, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, thickness=1)

        cv2.putText(img, str(frame), (1230, 710),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, WHITE, thickness=1)
        ...
        for box in boxes.itertuples(index=False):
            ...
            cv2.rectangle(
                ...
                (int(box.left_sub + box.width_sub),
                 int(box.top_sub + box.height_sub)),
                box_color,
    ...
    subprocess.run(
        ["ffmpeg", "-i", tmp_output_path, "-crf", "18", "-preset",
